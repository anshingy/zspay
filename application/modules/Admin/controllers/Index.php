...
{
	private $github_url = "https://github.com/zlkbdotnet/zspay/releases";
		...
		if($method AND $method=='updatecheck'){
			if ($this->VerifyCsrfToken($csrf_token)) {
				...
				if(version_compare(trim(VERSION), trim($up_version), '<' )){
					$params = array('update'=>1,'url'=>$this->github_url,'zip'=>"https://github.com/zlkbdotnet/zspay/archive/{$up_version}.zip");
	...
	private function _getUpdateVersion()
	{
		$version = VERSION;
		try{
			$version_reg = '#<a href="/zlkbdotnet/zspay/archive/(.*?).zip"#';//列表规则 
