...
class PcBasicController extends BasicController
{
	...
	public function _getProduct($hash_id)
	{
		$result = $this->_getCache($hash_id);
		if(is_array($result) AND isset($result['code']) AND $result['code']>0){
			$product = $result['data'];
		}else{
			
			$product = $this->load('products')->Where(array('hashid'=>$hash_id,'isdelete'=>0))->SelectOne();
			$this->_setCache($hash_id,$product);
		}
		return $product;
	}
	...
	public function _getCache($hash_id)
	{
		try {
			$redis = new Phpredis();
		} catch (\Exception $e) {
		...
		if($redis===false){
			$data = \Yaf\Session::getInstance()->__get($hash_id);
			if($data == null){
				$result = array('code'=>0,'msg'=>'no cache');
			}else{
				$result = array('code'=>1,'msg'=>'cache','data'=>$data);
			}
		}else{
			if($redis->exists($hash_id)){
				$data = $redis->get($hash_id);
				if($data == null){
					$result = array('code'=>0,'msg'=>'no cache');
				}else{
