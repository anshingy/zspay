...
class zfbf2f
{
	...
	public function pay($payconfig,$params)
	{
		$config = [
			...
			'return_url' => $params['web_url']. '/api/notify/?paymethod='.$this->paymethod.'&orderid='.$params['orderid'],
			'notify_url' => $params['web_url'] . '/api/notify/?paymethod='.$this->paymethod,
			'return_raw' => true
		];
		...
		try {
			...
			if($qr){
				$result_params = array('type'=>0,'paymethod'=>$this->paymethod,'qr'=>$qr,'payname'=>$payconfig['name'],'overtime'=>$payconfig['overtime'],'money'=>$params['money']);
